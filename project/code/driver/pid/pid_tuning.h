...
inline constexpr float kTargetCountMin = -1550.0f;
...
inline constexpr Profile make_normal_profile()
{
    ...
    profile.base_speed = 425.0f; // NORMAL 档基础速度：非直道加速阶段默认按 300 count/5ms 行驶。
    profile.straight_full_speed_error_threshold_px = 1.0f; // NORMAL 档直道满速阈值：整条路径均值误差小于 1px 才允许直通满速。

    profile.position_dynamic_kp_quad_a = 50000.0f; // NORMAL 档位置环动态Kp二次项：误差越大，Kp 增长越快。
    profile.position_dynamic_kp_base = 1520.0f; // NORMAL 档位置环动态Kp基础值：零误差附近从 1300 起步，避免“名义基础值”被下限吃掉。
    profile.position_dynamic_kp_min = 1520.0f; // NORMAL 档位置环动态Kp下限：当前与基础值保持一致，表示零误差起点就是最小 Kp。
    profile.position_dynamic_kp_max = 6000.0f; // NORMAL 档位置环动态Kp上限：防止比例项过强。
    profile.position_ki = 0.0f; // NORMAL 档位置环积分项：当前关闭，避免积分拖尾。
    profile.position_kd = 12.0f; // NORMAL 档位置环微分项：抑制转向过冲和来回摆动。
    profile.position_max_integral = 0.0f; // NORMAL 档位置环积分限幅：0 表示当前不启用额外积分限幅。
    profile.position_max_output = 2250.0f; // NORMAL 档位置环输出限幅：位置支路允许的最大差速。
    profile.steering_max_output = 2250.0f; // NORMAL 档总转向限幅：位置环与角速度环叠加后的总差速上限。
    ...
    profile.yaw_rate_kp = 4.7f; // NORMAL 档角速度环比例项：把弯道基础支撑补回来，避免入弯后车头跟不上。
    profile.yaw_rate_dynamic_kp_quad_a = 105.0f; // NORMAL 档角速度环动态Kp二次项：弯里误差一起来就更快抬 Kp，减少“发飘”。
    profile.yaw_rate_dynamic_kp_min = 4.7f; // NORMAL 档角速度环动态Kp下限：与基础 Kp 对齐，保证一进角速度环就有足够支撑。
    profile.yaw_rate_dynamic_kp_max = 18.0f; // NORMAL 档角速度环动态Kp上限：给中大弯留更强纠偏空间，但仍比固定大 Kp 更可控。
    profile.yaw_rate_kp_enable_error_threshold_px = 0.6f; // NORMAL 档角速度环Kp启用阈值：更早介入，避免等偏差放大后才开始补转向。
    ...
    profile.yaw_rate_max_output = 2260.0f; // NORMAL 档角速度环输出限幅：角速度支路允许的最大差速。
...
inline constexpr Profile make_straight_profile()
{
    ...
    profile.base_speed = 475.0f; // STRAIGHT 档基础速度：双边直道稳定时按更高基础速度巡线。
    profile.straight_full_speed_error_threshold_px = 1.5f; // STRAIGHT 档直道满速阈值：放宽到 1.5px，让稳定直道更容易持续加速。
    profile.position_dynamic_kp_quad_a = 2000.0f; // STRAIGHT 档位置环动态Kp二次项：高速直道下误差一变大就更快拉回中线。
    profile.position_dynamic_kp_base = 1200.0f; // STRAIGHT 档位置环动态Kp基础值：小误差时也保留更明确的回中线能力。
    profile.position_dynamic_kp_min = 1200.0f; // STRAIGHT 档位置环动态Kp下限：保证直道小误差区不把位置环“吃没了”。
    profile.position_dynamic_kp_max = 6500.0f; // STRAIGHT 档位置环动态Kp上限：给高速直道留出更强纠偏空间。
    profile.position_kd = 8.0f; // STRAIGHT 档位置环微分项：抑制高速回正时的来回摆动。
    profile.position_max_output = 1350.0f; // STRAIGHT 档位置环输出限幅：允许位置支路输出更大的差速修正。
    profile.steering_max_output = 1350.0f; // STRAIGHT 档总转向限幅：让增强后的位置环真正有空间打出来。
...
inline constexpr Profile make_circle_enter_profile()
{
    ...
    profile.base_speed = 360.0f; // 环岛入口档基础速度：入口默认按 225 count/5ms 行驶。
    profile.straight_full_speed_error_threshold_px = 0.0f; // 环岛入口档直道满速阈值：禁用入口阶段满速直通。

    profile.position_dynamic_kp_quad_a = 55000.0f; // 环岛入口档位置环动态Kp二次项：大误差时更积极地拉回中线。
    profile.position_dynamic_kp_base = 2000.0f; // 环岛入口档位置环动态Kp基础值：零误差附近就保持较强贴线，和实际生效起点一致。
    profile.position_dynamic_kp_min = 2000.0f; // 环岛入口档位置环动态Kp下限：当前与基础值一致，避免基础值被下限覆盖。
    ...
    profile.yaw_rate_kp = 4.5f; // 环岛入口档角速度环比例项：提高姿态跟踪强度，并保持在动态Kp允许区间内。
    profile.yaw_rate_dynamic_kp_quad_a = 110.0f; // 环岛入口档角速度环动态Kp二次项：默认关闭动态。
    profile.yaw_rate_dynamic_kp_min = 4.5f; // 环岛入口档角速度环动态Kp下限。
    profile.yaw_rate_dynamic_kp_max = 18.0f; // 环岛入口档角速度环动态Kp上限。
    profile.yaw_rate_max_output = 1273.0f; // 环岛入口档角速度环输出限幅：角速度支路可给更大差速。
...
inline constexpr Profile make_circle_inside_profile()
{
    ...
    profile.base_speed = 360.0f; // 环岛内部档基础速度：内部默认按 200 count/5ms 控速。
    profile.straight_full_speed_error_threshold_px = 0.0f; // 环岛内部档直道满速阈值：禁用内部阶段满速直通。

    profile.position_dynamic_kp_quad_a = 60000.0f; // 环岛内部档位置环动态Kp二次项：大误差时更果断地拉回单边中线。
    profile.position_dynamic_kp_base = 2400.0f; // 环岛内部档位置环动态Kp基础值：零误差附近也保持高强度贴线，和实际生效起点对齐。
    profile.position_dynamic_kp_min =2400.0f; // 环岛内部档位置环动态Kp下限：当前与基础值一致，避免环内小误差区出现隐藏夹限。
    ...
    profile.yaw_rate_kp = kNormalProfile.yaw_rate_kp; // 环岛内部档角速度环比例项：与 NORMAL 对齐。
    profile.yaw_rate_dynamic_kp_quad_a = kNormalProfile.yaw_rate_dynamic_kp_quad_a; // 环岛内部档角速度环动态Kp二次项：与 NORMAL 对齐。
    profile.yaw_rate_dynamic_kp_min = kNormalProfile.yaw_rate_dynamic_kp_min; // 环岛内部档角速度环动态Kp下限：与 NORMAL 对齐。
    profile.yaw_rate_dynamic_kp_max = kNormalProfile.yaw_rate_dynamic_kp_max; // 环岛内部档角速度环动态Kp上限：与 NORMAL 对齐。
    profile.yaw_rate_ki = kNormalProfile.yaw_rate_ki; // 环岛内部档角速度环积分项：与 NORMAL 对齐。
    profile.yaw_rate_kd = kNormalProfile.yaw_rate_kd; // 环岛内部档角速度环微分项：与 NORMAL 对齐。
    profile.yaw_rate_max_integral = kNormalProfile.yaw_rate_max_integral; // 环岛内部档角速度环积分限幅：与 NORMAL 对齐。
    profile.yaw_rate_max_output = kNormalProfile.yaw_rate_max_output; // 环岛内部档角速度环输出限幅：与 NORMAL 对齐。
...
inline constexpr Profile make_circle_exit_profile()
{
    ...
    profile.base_speed = 360.0f; // 环岛出环档基础速度：出环默认按 250 count/5ms 过渡回直道。
    profile.straight_full_speed_error_threshold_px = 0.0f; // 环岛出环档直道满速阈值：禁用出环阶段满速直通。

    profile.position_dynamic_kp_quad_a = 60000.0f; // 环岛出环档位置环动态Kp二次项：保持较强的大误差纠偏能力。
    ...
    profile.yaw_rate_kp = 4.5f; // 环岛出环档角速度环比例项：姿态跟踪比 NORMAL 略紧。
    profile.yaw_rate_dynamic_kp_quad_a = 110.0f; // 环岛出环档角速度环动态Kp二次项：默认关闭动态。
    profile.yaw_rate_dynamic_kp_min = 4.5f; // 环岛出环档角速度环动态Kp下限。
    profile.yaw_rate_dynamic_kp_max = 18.0f; // 环岛出环档角速度环动态Kp上限。
