...
inline constexpr Profile make_normal_profile()
{
    ...
    profile.position_dynamic_kp_quad_a = 1.5; // NORMAL 档位置环动态Kp一次项：误差越大，Kp 按线性速度增长。
    ...
    profile.steering_max_output = 400.0f; // NORMAL 档总转向限幅：位置环与角速度环叠加后的总差速上限。
    ...
    profile.yaw_rate_kp = 1.5f; // NORMAL 档角速度环比例项：把弯道基础支撑补回来，避免入弯后车头跟不上。
    ...
    profile.turn_slowdown_full_px = 20.0f; // NORMAL 档弯道降速满量程点：误差到 60px 时达到最大降速。
...
inline constexpr Profile make_straight_profile()
{
    ...
    // profile.position_dynamic_kp_quad_a = 80.0f; // STRAIGHT 档位置环动态Kp一次项：高速直道下误差一变大就更快拉回中线。
...
inline constexpr Profile make_circle_enter_profile()
{
    ...
    // profile.position_dynamic_kp_quad_a = 40.0f; // 环岛入口档位置环动态Kp一次项：大误差时更积极地拉回中线。
