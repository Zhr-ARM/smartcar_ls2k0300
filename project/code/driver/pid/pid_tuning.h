...
inline constexpr Profile make_normal_profile()
{
    ...
    profile.steering_max_output = 440.0f; // NORMAL 档总转向限幅：位置环与角速度环叠加后的总差速上限。
    ...
    profile.yaw_rate_ref_limit_dps = 300.0f; // NORMAL 档目标横摆角速度限幅：防止视觉异常时姿态目标过大。
    profile.yaw_rate_kp = 1.91f; // NORMAL 档角速度环比例项：把弯道基础支撑补回来，避免入弯后车头跟不上。
    ...
    profile.yaw_rate_kd = 0.025f; // NORMAL 档角速度环微分项：略回一点，帮助压住补强后可能带来的过冲。
    profile.yaw_rate_max_integral = 0.0f; // NORMAL 档角速度环积分限幅：0 表示当前不启用额外积分限幅。
    profile.yaw_rate_max_output = 280.0f; // NORMAL 档角速度环输出限幅：角速度支路允许的最大差速。
    ...
    profile.turn_min_speed_scale = 0.99f; // NORMAL 档最低保速比例：再大误差也至少保留 7% 基础速度。
