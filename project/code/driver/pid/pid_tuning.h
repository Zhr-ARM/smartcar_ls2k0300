...
inline constexpr Profile make_normal_profile()
{
    ...
    profile.position_dynamic_kp_quad_a = 4.6f; // NORMAL 档位置环动态Kp一次项：误差越大，Kp 按线性速度增长。
    profile.position_dynamic_kp_base = 1.6f; // NORMAL 档位置环动态Kp基础值：零误差附近从 1300 起步，避免“名义基础值”被下限吃掉。
    ...
    profile.yaw_rate_max_output = 290.0f; // NORMAL 档角速度环输出限幅：角速度支路允许的最大差速。
...
inline constexpr Profile make_straight_profile()
{
    ...
    profile.base_speed = 420.0f; // NORMAL 档基础速度：非直道加速阶段默认按 300 count/5ms 行驶。
    profile.straight_full_speed_error_threshold_px = 0.0f; // NORMAL 档直道满速阈值：整条路径均值误差小于 1px 才允许直通满速。
