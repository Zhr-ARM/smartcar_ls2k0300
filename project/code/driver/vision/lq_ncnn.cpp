...
std::string LQ_NCNN::Infer(const cv::Mat& bgr_image)
{
    ...
    if (ret != 0) {
        throw std::runtime_error("ex.input failed, ret=" + std::to_string(ret));
    }

    ncnn::Mat logits;
    ret = ex.extract(m_output_name.c_str(), logits);
    if (ret != 0) {
        throw std::runtime_error("ex.extract failed, ret=" + std::to_string(ret));
    }

    if (logits.w <= 0) {
        throw std::runtime_error("Invalid output logits shape.");
    }
    ...
    std::vector<float> prob(logits.w, 0.f);
    float sum_exp = 0.f;
    for (int i = 0; i < logits.w; ++i) {
        prob[i] = std::exp(logits[i] - max_logit);
        sum_exp += prob[i];
    }
    if (sum_exp <= 0.f) {
    ...
    // 排序拿 Top1
    std::vector<int> idx(logits.w);
    for (int i = 0; i < logits.w; ++i) idx[i] = i;
    std::sort(idx.begin(), idx.end(), [&](int a, int b) { return prob[a] > prob[b]; });

    // 返回 Top1 标签（保持你现有主程序兼容）
    int class_id = idx[0];
    float top1_prob = prob[class_id];
    auto t1 = std::chrono::steady_clock::now();
    double infer_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();

    const char* result_name = "unknown";
    if (class_id >= 0 && class_id < static_cast<int>(m_labels.size())) {
        result_name = m_labels[class_id].c_str();
    }
    else
    {
        // id越界时，仍打印数字id
        static thread_local std::string id_buf;
        id_buf = std::to_string(class_id);
        result_name = id_buf.c_str();
    }

    printf("[NCNN] result=%s\tprob=%.4f\ttime=%.2fms\n", result_name, top1_prob, infer_ms);
    return std::string(result_name);
}
