...
int main(int, char**) 
{
    ...
    printf("[VISION CFG] mode=%d max_fps=%u infer=%d client_send=%d screen=%d roi_capture=%d cam=%dx%d cross_en=%d ipm_calib=%dx%d ipm_out=%dx%d maze_row_ratio=%.4f maze_row=%d maze_fb_ratio=%.4f maze_fb=%d maze_gap_ratio=%.4f maze_gap=%d cross_probe_x_ratio=%.4f cross_probe_x=%d cross_probe_row_ratio=%.4f cross_probe_row=%d cross_probe_gap_ratio=%.4f cross_probe_gap=%d cross_corner_dist_ratio=%.4f cross_corner_dist=%d cross_begin_y_ratio=%.4f cross_begin_y=%d cross_exit_row_ratio=%.4f cross_exit_row=%d cross_tail_ratio=%.4f cross_tail=%d maze_hist_offset_ratio=%.4f maze_hist_offset=%d maze_x_min_ratio=%.4f maze_x_min=%d maze_x_max_ratio=%.4f maze_x_max=%d undistort=%d ipm_tri=%d ipm_resample=%d ipm_step=%.2f ipm_angle_step=%d ipm_straight_min_pts=%d ipm_straight_check=%d ipm_straight_min_cos=%.2f ipm_shift=%.2f center_post=%d center_tri=%d center_resample=%d center_kappa_en=%d center_step=%.2f line_src=%d line_method=%d line_fixed_idx=%d line_weighted_cnt=%u line_speed_k=%.4f line_speed_b=%.2f line_idx_min=%d line_idx_max=%d\r\n",
           ...
           g_vision_runtime_config.route_cross_enabled ? 1 : 0,
           g_vision_processor_config.ipm_calibration_src_width,
           g_vision_processor_config.ipm_calibration_src_height,
           vision_ipm_width(),
           vision_ipm_height(),
           ...
           static_cast<double>(g_vision_runtime_config.maze_start_min_boundary_gap_ratio),
           vision_runtime_config_maze_start_min_boundary_gap_px(),
           static_cast<double>(g_vision_runtime_config.cross_probe_start_x_ratio),
           vision_runtime_config_cross_probe_start_x_px(),
           static_cast<double>(g_vision_runtime_config.cross_probe_min_row_ratio),
           vision_runtime_config_cross_probe_min_row_px(),
           static_cast<double>(g_vision_runtime_config.cross_probe_max_gap_ratio),
           vision_runtime_config_cross_probe_max_gap_px(),
           static_cast<double>(g_vision_runtime_config.route_cross_corner_distance_ratio),
           vision_runtime_config_route_cross_corner_distance_px(),
           static_cast<double>(g_vision_runtime_config.route_cross_begin_corner_src_y_ratio),
           vision_runtime_config_route_cross_begin_corner_src_y_px(),
           static_cast<double>(g_vision_runtime_config.route_cross_stop_row_exit_ratio),
           vision_runtime_config_route_cross_stop_row_exit_px(),
           static_cast<double>(g_vision_runtime_config.cross_tail_extra_points_ratio),
           vision_runtime_config_cross_tail_extra_points(),
           static_cast<double>(g_vision_runtime_config.maze_history_start_offset_ratio),
           vision_runtime_config_maze_history_start_offset_px(),
