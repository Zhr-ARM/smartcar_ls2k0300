...
- 冻结角点（CROSS_1 → CROSS_2）：`g_cross_stage2_frozen_left/right_corner_*`
...
→ （可选）`extrapolate_cross_lower_boundary_inplace(left_pts/right_pts)`（仅 CIRCLE 状态）

**CROSS_1 时**：`aux → 上角点搜索(dir=5) → lower→upper 桥接线(y_step=2) → base+bridge+aux_tail 拼接`，回写 `left_pts/right_pts`

**CROSS_2 时**：`冻结角点 aux → 上角点搜索 → 上角点 x 竖直下延至 maze_start_row`，回写 `left_pts/right_pts`。上角点失败则该侧置空。

**CROSS_3 时**：`固定锚点(左{15,100},右{145,100}) + 跳变截断(x阈值15px,前推2点) → bridge+tail 拼接`，回写 `left_pts/right_pts`
...
- `NORMAL/STRAIGHT → CROSS_1`：`cross_entry_ready`（双下角点 + 角点后贴边 ≥5 行）
- `CROSS_1 → CROSS_2`：`cross_stage2_ready`（任一角点 y ≥ 75）
- `CROSS_2 → CROSS_3`：`cross_stage3_ready`（双侧 start_frame_wall_rows ≥ 1）
- `CROSS_3 → NORMAL`：`cross_exit_ready`（start_boundary_gap_x ∈ (0, 155)）
